...
if __name__ == '__main__':
    ...
    for i in range(len(loss_functions)):
        ...
        print("Count-Sketch vs. Median-Sketch (" + loss_labels[i] + ")         min = %.2f" % np.min(loss_improv_count_sketch) + " max = %.2f" % np.max(loss_improv_count_sketch))
        print("Cutoff Count-Sketch vs. Median-Sketch (" + loss_labels[i] + ")  min = %.2f" % np.min(loss_improv_cutoff) + " max = %.2f" % np.max(loss_improv_cutoff))
        ...
        ax.plot(space_percent, loss_sketch_mean, label="Vanilla Count-Sketch", linewidth=3, color=colors[0])        
        ax.plot(space_percent, loss_cutoff_mean, label="Cutoff Count-Sketch", linewidth=3, linestyle='dashed', color=colors[1])
        ax.plot(space_percent, loss_algo_mean, label="Cutoff Median-Sketch", linewidth=3, linestyle='dashdot', color=colors[4])
    
        # 95% confidence interval 
        plt.fill_between(
            space_percent, 
            loss_sketch_mean - 2*loss_sketch_std, 
            loss_sketch_mean + 2*loss_sketch_std, 
            color=colors[0],
            alpha=0.2,
        )
        plt.fill_between(
            space_percent, 
            loss_cutoff_mean - 2*loss_cutoff_std, 
            loss_cutoff_mean + 2*loss_cutoff_std, 
            color=colors[1],
            alpha=0.2,
        )
        plt.fill_between(
            space_percent, 
            loss_algo_mean - 2*loss_algo_std, 
            loss_algo_mean + 2*loss_algo_std, 
            color=colors[4],
            alpha=0.2,
        )
        ...
        ax.set_ylim(top=max(np.max(loss_sketch_mean), np.max(loss_algo_mean)) * 5)
